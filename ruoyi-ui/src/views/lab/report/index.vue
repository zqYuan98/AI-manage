...
export default {
  ...
  methods: {
    ...
    downloadArtifact(code) { const report = Object.assign({}, this.selected); const extensions = { JSON: 'json', MARKDOWN: 'md', WORD: 'docx', PDF: 'pdf' }; return downloadReportArtifact(report.id, code).then(blob => { if (!blobValidate(blob)) return blob.text().then(text => { let payload = {}; try { payload = JSON.parse(text) } catch (error) { payload = {} } throw new Error(payload.msg || '制品下载失败') }); return saveAs(blob, `${report.reportNo || `report-${report.id}`}.${extensions[code] || code.toLowerCase()}`) }).catch(error => this.$message.error(error && error.message ? error.message : '制品下载失败')) },
