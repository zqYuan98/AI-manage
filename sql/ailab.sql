...
CREATE TABLE IF NOT EXISTS `lab_task` (
 ...
 PRIMARY KEY (`id`), KEY `idx_lab_task_parent` (`parent_id`), KEY `idx_lab_task_goal` (`goal_id`), KEY `idx_lab_task_milestone` (`milestone_id`), KEY `idx_lab_task_owner_period_workflow` (`owner_id`,`period`,`workflow_status`), KEY `idx_lab_task_dept` (`dept_id`), KEY `idx_lab_task_coordination_owner` (`coordination_required`,`coordination_owner_id`), KEY `idx_lab_task_owner_status` (`owner_id`,`workflow_status`,`result_status`), KEY `idx_lab_task_period_line` (`period`,`biz_line`), KEY `idx_lab_task_asset` (`asset_id`)
...
CREATE TABLE IF NOT EXISTS `lab_reminder` (
 ...
 `active_unique_flag` tinyint GENERATED ALWAYS AS (CASE WHEN `del_flag` = '0' THEN 1 ELSE NULL END) STORED COMMENT 'active record unique marker',
 PRIMARY KEY (`id`), UNIQUE KEY `uk_lab_reminder_idempotency` (`idempotency_key`,`active_unique_flag`), KEY `idx_lab_reminder_recipient_read` (`recipient_id`,`read_flag`), KEY `idx_lab_reminder_task` (`task_id`)
...
CREATE TABLE IF NOT EXISTS `lab_perf_score` (
 ...
 `active_unique_flag` tinyint GENERATED ALWAYS AS (CASE WHEN `del_flag` = '0' THEN 1 ELSE NULL END) STORED COMMENT 'active record unique marker',
 PRIMARY KEY (`id`), UNIQUE KEY `uk_lab_perf_member_period_rev` (`member_id`,`period`,`revision_no`,`active_unique_flag`), KEY `idx_lab_perf_member_period_current` (`member_id`,`period`,`current_flag`), KEY `idx_lab_perf_period_current` (`period`,`current_flag`)
...
CREATE TABLE IF NOT EXISTS `lab_report_template` (
 ...
 `active_unique_flag` tinyint GENERATED ALWAYS AS (CASE WHEN `del_flag` = '0' THEN 1 ELSE NULL END) STORED COMMENT 'active record unique marker',
 PRIMARY KEY (`id`), UNIQUE KEY `uk_lab_report_tpl_code_rev` (`template_code`,`revision_no`,`active_unique_flag`), KEY `idx_lab_report_tpl_default` (`period_type`,`default_flag`,`status`)
...
CREATE TABLE IF NOT EXISTS `lab_report_instance` (
 ...
 `active_unique_flag` tinyint GENERATED ALWAYS AS (CASE WHEN `del_flag` = '0' THEN 1 ELSE NULL END) STORED COMMENT 'active record unique marker',
 PRIMARY KEY (`id`), UNIQUE KEY `uk_lab_report_instance_no` (`report_no`,`active_unique_flag`), UNIQUE KEY `uk_lab_report_instance_period_rev` (`template_id`,`period`,`biz_line`,`revision_no`,`active_unique_flag`), KEY `idx_lab_report_instance_tpl_period_lifecycle` (`template_id`,`period`,`lifecycle_status`), KEY `idx_lab_report_instance_period` (`period`,`biz_line`,`lifecycle_status`), KEY `idx_lab_report_instance_final` (`final_flag`,`current_flag`)
...
CREATE TABLE IF NOT EXISTS `lab_report_job` (
 ...
 `active_unique_flag` tinyint GENERATED ALWAYS AS (CASE WHEN `del_flag` = '0' THEN 1 ELSE NULL END) STORED COMMENT 'active record unique marker',
 PRIMARY KEY (`id`), UNIQUE KEY `uk_lab_report_job_no` (`job_no`,`active_unique_flag`), UNIQUE KEY `uk_lab_report_job_idempotency` (`idempotency_key`,`active_unique_flag`), KEY `idx_lab_report_job_instance_status` (`report_id`,`job_status`), KEY `idx_lab_report_job_type_status` (`job_type`,`job_status`)
...
(31090,'Close period',31009,1,'#','','','',1,0,'F','0','0','lab:perf:close','#','admin',NOW(),''),(31091,'Reopen period',31009,2,'#','','','',1,0,'F','0','0','lab:perf:reopen','#','admin',NOW(),''),(31092,'Red line',31009,3,'#','','','',1,0,'F','0','0','lab:perf:redline','#','admin',NOW(),''),(31093,'Revoke',31009,4,'#','','','',1,0,'F','0','0','lab:perf:revoke','#','admin',NOW(),''),(31094,'Calibrate',31009,5,'#','','','',1,0,'F','0','0','lab:perf:calibrate','#','admin',NOW(),''),
...
INSERT INTO `sys_role_menu` (`role_id`,`menu_id`) SELECT 30002,`menu_id` FROM `sys_menu` WHERE `menu_id` IN (31000,31001,31002,31003,31004,31005,31006,31007,31008,31009,31010,31011,31020,31021,31030,31031,31032,31033,31034,31070,31071,31072,31080,31081,31082,31090,31092,31094,31100,31110,31111,31112,31113);
INSERT INTO `sys_role_menu` (`role_id`,`menu_id`) SELECT 30003,`menu_id` FROM `sys_menu` WHERE `menu_id` IN (31000,31001,31002,31003,31004,31005,31006,31007,31008,31011,31030,31031,31032,31033,31070,31071,31072,31080,31081,31082,31112);
...
INSERT INTO `lab_report_template` (`id`,`template_code`,`template_name`,`period_type`,`revision_no`,`latest_flag`,`default_flag`,`status`,`header_json`,`style_json`,`version`,`create_by`,`create_time`,`remark`) VALUES (30001,'standard_month','Standard monthly laboratory report','MONTH',1,'1','1','ENABLED',JSON_OBJECT('title','AI Laboratory Monthly Report','logo','ai-lab'),JSON_OBJECT('theme','blue','font','Microsoft YaHei'),1,'admin',NOW(),'Default standard monthly template');
INSERT INTO `lab_report_section` (`id`,`template_id`,`section_code`,`section_name`,`section_type`,`sort_no`,`data_source`,`query_config_json`,`render_config_json`,`style_config_json`,`manual_flag`,`visible_flag`,`sensitive_flag`,`version`,`create_by`,`create_time`) VALUES (30001,30001,'TASK_TABLE','Task delivery','TABLE',10,'TASK_DELIVERY',JSON_OBJECT('period','${period}'),JSON_OBJECT('columns',JSON_ARRAY('owner','status','deliverable')),JSON_OBJECT('width','100%'),'0','1','0',1,'admin',NOW()),(30002,30001,'SCORE_STAT','Performance overview','STAT',20,'PERFORMANCE_SCORE',JSON_OBJECT('period','${period}'),JSON_OBJECT('metrics',JSON_ARRAY('average','top')),JSON_OBJECT(),'0','1','1',1,'admin',NOW()),(30003,30001,'MANAGER_TEXT','Manager narrative','TEXT',30,'GOAL_PROGRESS',JSON_OBJECT(),JSON_OBJECT('template','Goal progress summary'),JSON_OBJECT(),'0','1','0',1,'admin',NOW()),(30004,30001,'MANUAL_NOTE','Manual note','MANUAL',40,NULL,JSON_OBJECT(),JSON_OBJECT('placeholder','Enter management note'),JSON_OBJECT(),'1','1','0',1,'admin',NOW()),(30005,30001,'LINE_GROUP','Line updates','GROUP_TEXT',50,'LINE_SUMMARY',JSON_OBJECT('groupBy','biz_line'),JSON_OBJECT('template','line update'),JSON_OBJECT(),'0','1','0',1,'admin',NOW()),(30006,30001,'PROGRESS_CHART','Goal progress','CHART',60,'GOAL_PROGRESS',JSON_OBJECT(),JSON_OBJECT('chart','bar'),JSON_OBJECT(),'0','1','0',1,'admin',NOW());
INSERT INTO `lab_report_summary` (`id`,`period`,`biz_line`,`section_code`,`summary_json`,`summary_text`,`source_revision`,`create_by`,`create_time`) VALUES (30001,'2026-07','algorithm','LINE_GROUP',JSON_OBJECT('completed',4,'blocked',0),'Algorithm work met July targets',1,'admin',NOW()),(30002,'2026-08','platform','TASK_TABLE',JSON_OBJECT('active',2,'blocked',2),'Platform release needs metric coordination',1,'admin',NOW());
INSERT INTO `lab_report_instance` (`id`,`report_no`,`template_id`,`period`,`biz_line`,`revision_no`,`lifecycle_status`,`current_flag`,`final_flag`,`sensitive_flag`,`source_data_json`,`source_perf_revision`,`content_json`,`content_markdown`,`json_status`,`json_path`,`json_error`,`markdown_status`,`markdown_path`,`markdown_error`,`word_status`,`word_path`,`word_error`,`pdf_status`,`pdf_path`,`pdf_error`,`version`,`create_by`,`create_time`) VALUES (30001,'RPT-2026-07-ALL',30001,'2026-07','ALL',1,'FINAL','1','1','1',JSON_OBJECT('source','summary'),1,JSON_OBJECT('sections',6),'# AI Laboratory Monthly Report\nJuly closed report','READY','/reports/2026-07.json',NULL,'READY','/reports/2026-07.md',NULL,'READY','/reports/2026-07.docx',NULL,'READY','/reports/2026-07.pdf',NULL,1,'admin',NOW());
