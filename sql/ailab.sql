...
SET @ailab_ddl = (SELECT IF(COUNT(*)=0, 'ALTER TABLE `lab_goal` ADD COLUMN `termination_reason` varchar(500) NULL COMMENT ''required termination reason'' AFTER `status`', 'SELECT 1') FROM information_schema.columns WHERE table_schema=DATABASE() AND table_name='lab_goal' AND column_name='termination_reason');
PREPARE ailab_ddl FROM @ailab_ddl; EXECUTE ailab_ddl; DEALLOCATE PREPARE ailab_ddl;
SET @ailab_ddl = (SELECT IF(COUNT(*)=0, 'ALTER TABLE `lab_goal` ADD COLUMN `terminated_by` bigint NULL COMMENT ''manager who terminated the goal'' AFTER `termination_reason`', 'SELECT 1') FROM information_schema.columns WHERE table_schema=DATABASE() AND table_name='lab_goal' AND column_name='terminated_by');
PREPARE ailab_ddl FROM @ailab_ddl; EXECUTE ailab_ddl; DEALLOCATE PREPARE ailab_ddl;
SET @ailab_ddl = (SELECT IF(COUNT(*)=0, 'ALTER TABLE `lab_goal` ADD COLUMN `terminated_time` datetime NULL COMMENT ''goal termination time'' AFTER `terminated_by`', 'SELECT 1') FROM information_schema.columns WHERE table_schema=DATABASE() AND table_name='lab_goal' AND column_name='terminated_time');
PREPARE ailab_ddl FROM @ailab_ddl; EXECUTE ailab_ddl; DEALLOCATE PREPARE ailab_ddl;
...
PREPARE ailab_ddl FROM @ailab_ddl; EXECUTE ailab_ddl; DEALLOCATE PREPARE ailab_ddl;
UPDATE `lab_reminder` SET `business_id`=COALESCE(`business_id`,`task_id`),`reminder_date`=COALESCE(`reminder_date`,DATE(`send_time`),DATE(`create_time`),CURRENT_DATE),`title`=COALESCE(NULLIF(`title`,''),'实验室提醒') WHERE `business_id` IS NULL OR `reminder_date` IS NULL OR `title` IS NULL OR `title`='';
...
PREPARE ailab_ddl FROM @ailab_ddl; EXECUTE ailab_ddl; DEALLOCATE PREPARE ailab_ddl;
SET @ailab_ddl=(SELECT IF(COUNT(*)=0,'ALTER TABLE `lab_report_instance` ADD COLUMN `source_close_revision` bigint NULL COMMENT ''pinned period close snapshot'' AFTER `source_perf_revision`','SELECT 1') FROM information_schema.columns WHERE table_schema=DATABASE() AND table_name='lab_report_instance' AND column_name='source_close_revision');
PREPARE ailab_ddl FROM @ailab_ddl; EXECUTE ailab_ddl; DEALLOCATE PREPARE ailab_ddl;
SET @ailab_ddl=(SELECT IF(COUNT(*)=0,'ALTER TABLE `lab_report_instance` ADD COLUMN `source_formal_revision` bigint NULL COMMENT ''pinned formal acceptance revision'' AFTER `source_close_revision`','SELECT 1') FROM information_schema.columns WHERE table_schema=DATABASE() AND table_name='lab_report_instance' AND column_name='source_formal_revision');
PREPARE ailab_ddl FROM @ailab_ddl; EXECUTE ailab_ddl; DEALLOCATE PREPARE ailab_ddl;
SET @ailab_ddl=(SELECT IF(COUNT(*)=0,'ALTER TABLE `lab_report_instance` ADD COLUMN `source_execution_cutoff` datetime NULL COMMENT ''pinned contextual fact cutoff'' AFTER `source_formal_revision`','SELECT 1') FROM information_schema.columns WHERE table_schema=DATABASE() AND table_name='lab_report_instance' AND column_name='source_execution_cutoff');
PREPARE ailab_ddl FROM @ailab_ddl; EXECUTE ailab_ddl; DEALLOCATE PREPARE ailab_ddl;
SET @ailab_ddl=(SELECT IF(COUNT(*)=0,'ALTER TABLE `lab_report_instance` ADD COLUMN `preview_only` char(1) NOT NULL DEFAULT ''0'' COMMENT ''live preview can never be finalized'' AFTER `source_execution_cutoff`','SELECT 1') FROM information_schema.columns WHERE table_schema=DATABASE() AND table_name='lab_report_instance' AND column_name='preview_only');
PREPARE ailab_ddl FROM @ailab_ddl; EXECUTE ailab_ddl; DEALLOCATE PREPARE ailab_ddl;
SET @ailab_ddl=(SELECT IF(COUNT(*)=0,'ALTER TABLE `lab_report_instance` ADD COLUMN `json_hash` char(64) NULL COMMENT ''final JSON SHA-256'' AFTER `preview_only`','SELECT 1') FROM information_schema.columns WHERE table_schema=DATABASE() AND table_name='lab_report_instance' AND column_name='json_hash');
PREPARE ailab_ddl FROM @ailab_ddl; EXECUTE ailab_ddl; DEALLOCATE PREPARE ailab_ddl;
SET @ailab_ddl=(SELECT IF(COUNT(*)=0,'ALTER TABLE `lab_report_instance` ADD COLUMN `markdown_hash` char(64) NULL COMMENT ''final Markdown SHA-256'' AFTER `json_hash`','SELECT 1') FROM information_schema.columns WHERE table_schema=DATABASE() AND table_name='lab_report_instance' AND column_name='markdown_hash');
PREPARE ailab_ddl FROM @ailab_ddl; EXECUTE ailab_ddl; DEALLOCATE PREPARE ailab_ddl;
SET @ailab_ddl=(SELECT IF(COUNT(*)=0,'ALTER TABLE `lab_report_instance` ADD COLUMN `word_hash` char(64) NULL COMMENT ''final Word SHA-256'' AFTER `markdown_hash`','SELECT 1') FROM information_schema.columns WHERE table_schema=DATABASE() AND table_name='lab_report_instance' AND column_name='word_hash');
PREPARE ailab_ddl FROM @ailab_ddl; EXECUTE ailab_ddl; DEALLOCATE PREPARE ailab_ddl;
SET @ailab_ddl=(SELECT IF(COUNT(*)=0,'ALTER TABLE `lab_report_instance` ADD COLUMN `pdf_hash` char(64) NULL COMMENT ''final PDF SHA-256'' AFTER `word_hash`','SELECT 1') FROM information_schema.columns WHERE table_schema=DATABASE() AND table_name='lab_report_instance' AND column_name='pdf_hash');
PREPARE ailab_ddl FROM @ailab_ddl; EXECUTE ailab_ddl; DEALLOCATE PREPARE ailab_ddl;
...
(30001,'业务线','lab_biz_line','0','admin',NOW(),'人工智能实验室'),(30002,'任务流程状态','lab_task_workflow_status','0','admin',NOW(),'人工智能实验室'),(30003,'任务结果','lab_task_result_status','0','admin',NOW(),'人工智能实验室'),(30004,'任务类型','lab_task_type','0','admin',NOW(),'人工智能实验室'),(30005,'任务层级','lab_task_level','0','admin',NOW(),'人工智能实验室'),(30006,'资产类型','lab_asset_type','0','admin',NOW(),'人工智能实验室'),(30007,'资产阶段','lab_asset_stage','0','admin',NOW(),'人工智能实验室'),(30008,'知识产权类型','lab_ipr_type','0','admin',NOW(),'人工智能实验室'),(30009,'知识产权阶段','lab_ipr_stage','0','admin',NOW(),'人工智能实验室'),(30010,'报告章节类型','lab_section_type','0','admin',NOW(),'人工智能实验室'),(30011,'目标状态','lab_goal_status','0','admin',NOW(),'人工智能实验室'),(30012,'报告状态','lab_report_status','0','admin',NOW(),'人工智能实验室'),(30013,'报告任务状态','lab_report_job_status','0','admin',NOW(),'人工智能实验室'),(30014,'报告制品状态','lab_artifact_status','0','admin',NOW(),'人工智能实验室'),(30015,'审核状态','lab_review_status','0','admin',NOW(),'人工智能实验室'),(30016,'协同类别','lab_collaboration_category','0','admin',NOW(),'人工智能实验室'),(30017,'周承诺执行状态','lab_task_execution_status','0','admin',NOW(),'人工智能实验室');
...
(30101,1,'硬件设备','hardware','lab_biz_line','N','0','admin',NOW()),(30102,2,'工程研发','platform','lab_biz_line','N','0','admin',NOW()),(30103,3,'算法研发','algorithm','lab_biz_line','N','0','admin',NOW()),(30104,4,'部门管理','manage','lab_biz_line','N','0','admin',NOW()),
(30110,1,'草稿','DRAFT','lab_task_workflow_status','N','0','admin',NOW()),(30111,2,'进行中','ACTIVE','lab_task_workflow_status','N','0','admin',NOW()),(30112,3,'待验收','PENDING_REVIEW','lab_task_workflow_status','N','0','admin',NOW()),(30113,4,'已确认','CONFIRMED','lab_task_workflow_status','N','0','admin',NOW()),
(30120,1,'进行中','DOING','lab_task_result_status','N','0','admin',NOW()),(30121,2,'超额完成','EXCEEDED','lab_task_result_status','N','0','admin',NOW()),(30122,3,'按时完成','ONTIME','lab_task_result_status','N','0','admin',NOW()),(30123,4,'延期完成','DELAYED','lab_task_result_status','N','0','admin',NOW()),(30124,5,'未完成','UNDONE','lab_task_result_status','N','0','admin',NOW()),
...
(30130,1,'重点任务','key','lab_task_type','N','0','admin',NOW()),(30131,2,'日常任务','daily','lab_task_type','N','0','admin',NOW()),(30140,1,'月度任务','month','lab_task_level','N','0','admin',NOW()),(30141,2,'周承诺','week','lab_task_level','N','0','admin',NOW()),
(30150,1,'硬件设备','hardware','lab_asset_type','N','0','admin',NOW()),(30151,2,'算法资产','algorithm','lab_asset_type','N','0','admin',NOW()),(30152,3,'平台资产','platform','lab_asset_type','N','0','admin',NOW()),
(30160,1,'验证中','VERIFYING','lab_asset_stage','N','0','admin',NOW()),(30161,2,'已部署','DEPLOYED','lab_asset_stage','N','0','admin',NOW()),(30162,3,'已验收','ACCEPTED','lab_asset_stage','N','0','admin',NOW()),
(30170,1,'软件著作权','SOFTWARE_COPYRIGHT','lab_ipr_type','N','0','admin',NOW()),(30171,2,'专利','PATENT','lab_ipr_type','N','0','admin',NOW()),(30172,3,'认证','CERTIFICATION','lab_ipr_type','N','0','admin',NOW()),
(30180,1,'草稿','DRAFT','lab_ipr_stage','N','0','admin',NOW()),(30181,2,'准备中','PREPARING','lab_ipr_stage','N','0','admin',NOW()),(30182,3,'已提交','SUBMITTED','lab_ipr_stage','N','0','admin',NOW()),(30183,4,'已受理','ACCEPTED','lab_ipr_stage','N','0','admin',NOW()),(30184,5,'已授权','AUTHORIZED','lab_ipr_stage','N','0','admin',NOW()),
(30190,1,'表格','TABLE','lab_section_type','N','0','admin',NOW()),(30191,2,'统计指标','STAT','lab_section_type','N','0','admin',NOW()),(30192,3,'文本','TEXT','lab_section_type','N','0','admin',NOW()),(30193,4,'人工填写','MANUAL','lab_section_type','N','0','admin',NOW()),(30194,5,'分组文本','GROUP_TEXT','lab_section_type','N','0','admin',NOW()),(30195,6,'图表','CHART','lab_section_type','N','0','admin',NOW()),
(30200,1,'进行中','ACTIVE','lab_goal_status','N','0','admin',NOW()),(30201,2,'已完成','COMPLETED','lab_goal_status','N','0','admin',NOW()),(30202,3,'已终止','TERMINATED','lab_goal_status','N','0','admin',NOW()),
(30210,1,'草稿','DRAFT','lab_report_status','N','0','admin',NOW()),(30211,2,'已定稿','FINALIZED','lab_report_status','N','0','admin',NOW()),(30212,3,'历史版本','SUPERSEDED','lab_report_status','N','0','admin',NOW()),(30220,1,'排队中','QUEUED','lab_report_job_status','N','0','admin',NOW()),(30221,2,'执行中','RUNNING','lab_report_job_status','N','0','admin',NOW()),(30222,3,'已成功','SUCCESS','lab_report_job_status','N','0','admin',NOW()),(30223,4,'已失败','FAILED','lab_report_job_status','N','0','admin',NOW()),(30230,1,'待生成','PENDING','lab_artifact_status','N','0','admin',NOW()),(30231,2,'已生成','SUCCESS','lab_artifact_status','N','0','admin',NOW()),(30232,3,'生成失败','FAILED','lab_artifact_status','N','0','admin',NOW()),(30233,4,'未生成','NOT_REQUESTED','lab_artifact_status','N','0','admin',NOW()),(30240,1,'待审核','PENDING','lab_review_status','N','0','admin',NOW()),(30241,2,'已通过','APPROVED','lab_review_status','N','0','admin',NOW()),(30242,3,'已退回','REJECTED','lab_review_status','N','0','admin',NOW()),
(30250,1,'跨部门支持','CROSS_DEPT','lab_collaboration_category','N','0','admin',NOW()),(30251,2,'知识分享','KNOWLEDGE','lab_collaboration_category','N','0','admin',NOW()),(30252,3,'备份培养','BACKUP','lab_collaboration_category','N','0','admin',NOW()),(30253,4,'逾期扣分','OVERDUE','lab_collaboration_category','N','0','admin',NOW()),(30254,5,'人工扣分','DEDUCTION','lab_collaboration_category','N','0','admin',NOW());

-- Menu and permission tree; the 300xx range does not overlap the RuoYi baseline seed.
DELETE FROM `sys_role_menu` WHERE `menu_id` IN (31000,31001,31002,31003,31004,31005,31006,31007,31008,31009,31010,31011,31020,31021,31022,31023,31024,31030,31031,31032,31033,31034,31040,31041,31042,31050,31060,31061,31070,31071,31072,31080,31081,31082,31090,31091,31092,31093,31094,31095,31100,31101,31102,31110,31111,31112,31113,31114,31120,31121);
...
(31000,'部门管理',0,10,'lab',NULL,'','',1,0,'M','0','0','','dashboard','admin',NOW(),'部门管理根菜单'),
(31001,'管理看板',31000,1,'dashboard','lab/dashboard/index','','',1,0,'C','0','0','lab:dashboard:view','dashboard','admin',NOW(),''),(31002,'目标管理',31000,2,'goal','lab/goal/index','','',1,0,'C','0','0','lab:goal:list','target','admin',NOW(),''),(31003,'任务与承诺',31000,3,'task','lab/task/index','','',1,0,'C','0','0','lab:task:list','list','admin',NOW(),''),(31004,'成员档案',31000,4,'member','lab/member/index','','',1,0,'C','0','0','lab:member:list','user','admin',NOW(),''),(31005,'技能矩阵',31000,5,'skill','lab/member/index','','',1,0,'C','0','0','lab:skill:list','star','admin',NOW(),''),(31006,'一对一沟通',31000,6,'one2one','lab/member/index','','',1,0,'C','0','0','lab:one2one:list','chat-dot-round','admin',NOW(),''),(31007,'资产管理',31000,7,'asset','lab/asset/index','','',1,0,'C','0','0','lab:asset:list','cpu','admin',NOW(),''),(31008,'知识产权',31000,8,'ipr','lab/ipr/index','','',1,0,'C','0','0','lab:ipr:list','documentation','admin',NOW(),''),(31009,'绩效校准',31000,9,'perf','lab/performance/index','','',1,0,'C','0','0','lab:perf:list','trend-charts','admin',NOW(),''),(31010,'报告模板',31000,10,'template','lab/template/index','','',1,0,'C','0','0','lab:template:list','form','admin',NOW(),''),(31011,'月报归档',31000,11,'report','lab/report/index','','',1,0,'C','0','0','lab:report:list','document','admin',NOW(),''),
(31020,'新增目标',31002,1,'#','','','',1,0,'F','0','0','lab:goal:add','#','admin',NOW(),''),(31021,'编辑目标',31002,2,'#','','','',1,0,'F','0','0','lab:goal:edit','#','admin',NOW(),''),(31022,'删除目标',31002,3,'#','','','',1,0,'F','0','0','lab:goal:remove','#','admin',NOW(),''),(31023,'激活目标',31002,4,'#','','','',1,0,'F','0','0','lab:goal:activate','#','admin',NOW(),''),(31024,'终止目标',31002,5,'#','','','',1,0,'F','0','0','lab:goal:terminate','#','admin',NOW(),''),
(31030,'新增任务',31003,1,'#','','','',1,0,'F','0','0','lab:task:add','#','admin',NOW(),''),(31031,'编辑任务',31003,2,'#','','','',1,0,'F','0','0','lab:task:edit','#','admin',NOW(),''),(31032,'删除任务',31003,3,'#','','','',1,0,'F','0','0','lab:task:remove','#','admin',NOW(),''),(31033,'任务证据',31003,4,'#','','','',1,0,'F','0','0','lab:task:evidence','#','admin',NOW(),''),(31034,'任务验收',31003,5,'#','','','',1,0,'F','0','0','lab:task:review','#','admin',NOW(),''),
(31040,'新增成员',31004,1,'#','','','',1,0,'F','0','0','lab:member:add','#','admin',NOW(),''),(31041,'编辑成员',31004,2,'#','','','',1,0,'F','0','0','lab:member:edit','#','admin',NOW(),''),(31042,'停用成员',31004,3,'#','','','',1,0,'F','0','0','lab:member:remove','#','admin',NOW(),''),(31050,'技能配置',31005,1,'#','','','',1,0,'F','0','0','lab:skill:config','#','admin',NOW(),''),(31060,'新增一对一沟通',31006,1,'#','','','',1,0,'F','0','0','lab:one2one:add','#','admin',NOW(),''),(31061,'编辑一对一沟通',31006,2,'#','','','',1,0,'F','0','0','lab:one2one:edit','#','admin',NOW(),''),
(31070,'新增资产',31007,1,'#','','','',1,0,'F','0','0','lab:asset:add','#','admin',NOW(),''),(31071,'编辑资产',31007,2,'#','','','',1,0,'F','0','0','lab:asset:edit','#','admin',NOW(),''),(31072,'停用资产',31007,3,'#','','','',1,0,'F','0','0','lab:asset:remove','#','admin',NOW(),''),(31080,'新增知识产权',31008,1,'#','','','',1,0,'F','0','0','lab:ipr:add','#','admin',NOW(),''),(31081,'编辑知识产权',31008,2,'#','','','',1,0,'F','0','0','lab:ipr:edit','#','admin',NOW(),''),(31082,'停用知识产权',31008,3,'#','','','',1,0,'F','0','0','lab:ipr:remove','#','admin',NOW(),''),
(31090,'关闭绩效周期',31009,1,'#','','','',1,0,'F','0','0','lab:perf:close','#','admin',NOW(),''),(31091,'重开绩效周期',31009,2,'#','','','',1,0,'F','0','0','lab:perf:reopen','#','admin',NOW(),''),(31092,'登记红线',31009,3,'#','','','',1,0,'F','0','0','lab:perf:redline','#','admin',NOW(),''),(31093,'撤销红线',31009,4,'#','','','',1,0,'F','0','0','lab:perf:revoke','#','admin',NOW(),''),(31094,'季度校准',31009,5,'#','','','',1,0,'F','0','0','lab:perf:calibrate','#','admin',NOW(),''),(31095,'绩效历史',31009,6,'#','','','',1,0,'F','0','0','lab:perf:history','#','admin',NOW(),''),
(31100,'配置模板',31010,1,'#','','','',1,0,'F','0','0','lab:template:config','#','admin',NOW(),''),(31101,'导入模板',31010,2,'#','','','',1,0,'F','0','0','lab:template:import','#','admin',NOW(),''),(31102,'导出模板',31010,3,'#','','','',1,0,'F','0','0','lab:template:export','#','admin',NOW(),''),(31110,'生成报告',31011,1,'#','','','',1,0,'F','0','0','lab:report:generate','#','admin',NOW(),''),(31111,'重试报告',31011,2,'#','','','',1,0,'F','0','0','lab:report:retry','#','admin',NOW(),''),(31112,'下载报告',31011,3,'#','','','',1,0,'F','0','0','lab:report:download','#','admin',NOW(),''),(31113,'定稿报告',31011,4,'#','','','',1,0,'F','0','0','lab:report:finalize','#','admin',NOW(),''),(31114,'敏感报告',31011,5,'#','','','',1,0,'F','0','0','lab:report:sensitive','#','admin',NOW(),''),
(31120,'提醒列表',31001,1,'#','','','',1,0,'F','0','0','lab:reminder:list','#','admin',NOW(),''),(31121,'标记提醒已读',31001,2,'#','','','',1,0,'F','0','0','lab:reminder:read','#','admin',NOW(),'');
...
INSERT INTO `sys_role` (`role_id`,`role_name`,`role_key`,`role_sort`,`data_scope`,`menu_check_strictly`,`dept_check_strictly`,`status`,`del_flag`,`create_by`,`create_time`,`remark`) VALUES (30001,'部门负责人','lab_manager',1,1,1,1,'0','0','admin',NOW(),'部门完整管理权限'),(30002,'业务线负责人','lab_lead',2,2,1,1,'0','0','admin',NOW(),'本业务线管理权限'),(30003,'部门成员','lab_member',3,5,1,1,'0','0','admin',NOW(),'成员个人工作权限');
...
(30001,100,'lab_manager','部门负责人','00','lab.manager@example.test','','0','','$2a$10$7JB720yubVSZvUI0rEqK/.VqGOZTH.ulu33dHOiBE8ByOhJIrdAu0','1','0','admin',NOW(),'演示账号默认停用，管理员重置密码后方可启用'),
(30002,101,'lab_algorithm','算法负责人','00','algorithm@example.test','','0','','$2a$10$7JB720yubVSZvUI0rEqK/.VqGOZTH.ulu33dHOiBE8ByOhJIrdAu1','1','0','admin',NOW(),'演示账号默认停用，管理员重置密码后方可启用'),
(30003,101,'lab_platform','平台负责人','00','platform@example.test','','0','','$2a$10$7JB720yubVSZvUI0rEqK/.VqGOZTH.ulu33dHOiBE8ByOhJIrdAu3','1','0','admin',NOW(),'演示账号默认停用，管理员重置密码后方可启用'),
(30004,101,'lab_hardware','硬件负责人','00','hardware@example.test','','0','','$2a$10$7JB720yubVSZvUI0rEqK/.VqGOZTH.ulu33dHOiBE8ByOhJIrdAu4','1','0','admin',NOW(),'演示账号默认停用，管理员重置密码后方可启用'),
(30005,101,'lab_developer','平台工程师','00','developer@example.test','','0','','$2a$10$7JB720yubVSZvUI0rEqK/.VqGOZTH.ulu33dHOiBE8ByOhJIrdAu5','1','0','admin',NOW(),'演示账号默认停用，管理员重置密码后方可启用'),
(30006,101,'lab_researcher','算法工程师','00','researcher@example.test','','0','','$2a$10$7JB720yubVSZvUI0rEqK/.VqGOZTH.ulu33dHOiBE8ByOhJIrdAu6','1','0','admin',NOW(),'演示账号默认停用，管理员重置密码后方可启用');
...
DELETE FROM `lab_task` WHERE `id` IN (30001,30002,30003,30004,30005,30006);
...
INSERT INTO `lab_member` (`id`,`user_id`,`member_no`,`position`,`biz_line`,`role_type`,`leader_id`,`primary_responsibilities`,`backup_responsibilities`,`join_date`,`member_status`,`create_by`,`create_time`,`remark`) VALUES (30001,30001,'MGR-001','部门负责人','manage','MANAGER',NULL,'部门目标、资源与成员管理','月度关期与报告治理','2024-01-01','ACTIVE','admin',NOW(),'演示部门负责人');
INSERT INTO `lab_member` (`id`,`user_id`,`member_no`,`position`,`biz_line`,`role_type`,`leader_id`,`primary_responsibilities`,`backup_responsibilities`,`join_date`,`member_status`,`create_by`,`create_time`,`remark`) VALUES (30002,30002,'ALG-001','算法负责人','algorithm','LINE_LEAD',30001,'算法交付与评测','平台服务方案评审','2024-01-01','ACTIVE','admin',NOW(),'演示算法负责人');
INSERT INTO `lab_member` (`id`,`user_id`,`member_no`,`position`,`biz_line`,`role_type`,`leader_id`,`primary_responsibilities`,`backup_responsibilities`,`join_date`,`member_status`,`create_by`,`create_time`,`remark`) VALUES (30003,30003,'PLT-001','平台负责人','platform','LINE_LEAD',30001,'人工智能平台与模型服务','算法部署方案评审','2024-01-01','ACTIVE','admin',NOW(),'演示平台负责人');
INSERT INTO `lab_member` (`id`,`user_id`,`member_no`,`position`,`biz_line`,`role_type`,`leader_id`,`primary_responsibilities`,`backup_responsibilities`,`join_date`,`member_status`,`create_by`,`create_time`,`remark`) VALUES (30004,30004,'HW-001','硬件负责人','hardware','LINE_LEAD',30001,'加速器与设备集成','训练集群运维','2024-01-01','ACTIVE','admin',NOW(),'演示硬件负责人');
INSERT INTO `lab_member` (`id`,`user_id`,`member_no`,`position`,`biz_line`,`role_type`,`leader_id`,`primary_responsibilities`,`backup_responsibilities`,`join_date`,`member_status`,`create_by`,`create_time`,`remark`) VALUES (30005,30005,'PLT-002','平台工程师','platform','MEMBER',30003,'模型服务平台研发','模型工程备份','2025-01-01','ACTIVE','admin',NOW(),'演示平台成员');
INSERT INTO `lab_member` (`id`,`user_id`,`member_no`,`position`,`biz_line`,`role_type`,`leader_id`,`primary_responsibilities`,`backup_responsibilities`,`join_date`,`member_status`,`create_by`,`create_time`,`remark`) VALUES (30006,30006,'ALG-002','算法工程师','algorithm','MEMBER',30002,'模型评测实验','评测流水线备份','2025-01-01','ACTIVE','admin',NOW(),'演示算法成员');

INSERT INTO `lab_skill` (`id`,`skill_code`,`skill_name`,`skill_category`,`skill_desc`,`status`,`create_by`,`create_time`) VALUES (30001,'LLM','大语言模型','algorithm','模型训练与评测','ACTIVE','admin',NOW()),(30002,'MLOPS','模型工程','platform','模型服务与可观测性','ACTIVE','admin',NOW()),(30003,'EMBEDDED','嵌入式系统','hardware','设备与加速器集成','ACTIVE','admin',NOW());
...
INSERT INTO `lab_asset` (`id`,`asset_no`,`asset_name`,`asset_type`,`asset_stage`,`primary_owner_id`,`backup_owner_id`,`resource_url`,`repository_url`,`capacity_desc`,`status`,`create_by`,`create_time`) VALUES (30001,'AST-GPU-01','GPU 训练集群','hardware','ACCEPTED',30004,30003,'https://example.invalid/assets/gpu-cluster',NULL,'8 个 GPU 训练节点','ACTIVE','admin',NOW()),(30002,'AST-PLT-01','模型服务平台','platform','DEPLOYED',30003,30005,'https://example.invalid/assets/platform','https://example.invalid/repos/platform','支持多租户推理服务','ACTIVE','admin',NOW()),(30003,'AST-ALG-01','模型评测流水线','algorithm','VERIFYING',30002,30006,'https://example.invalid/assets/evaluation','https://example.invalid/repos/evaluation','回归评测套件','ACTIVE','admin',NOW());
INSERT INTO `lab_goal` (`id`,`parent_id`,`goal_level`,`year`,`period`,`goal_no`,`title`,`target_value`,`accept_criteria`,`owner_id`,`weight`,`progress_mode`,`progress_rate`,`progress_desc`,`status`,`version`,`create_by`,`create_time`) VALUES (30001,0,'YEAR',2026,NULL,'G-2026-01','建设稳定可靠的人工智能平台','可用性达到 99.5%','通过季度验收评审',30001,100,'MANUAL',62,'平台与模型能力持续建设中','ACTIVE',1,'admin',NOW()),(30002,30001,'QUARTER',2026,'2026Q3','G-2026Q3-ALG','提升模型质量','评测得分不低于 90','模型评测报告通过验收',30002,35,'MANUAL',75,'评测流水线已就绪','ACTIVE',1,'admin',NOW()),(30003,30001,'QUARTER',2026,'2026Q3','G-2026Q3-PLT','扩展模型服务能力','吞吐达到 500 QPS','负载测试通过验收',30003,35,'MANUAL',58,'发布加固正在进行','ACTIVE',1,'admin',NOW()),(30004,30001,'QUARTER',2026,'2026Q3','G-2026Q3-HW','完成加速器集群验收','8 个节点全部上线','硬件验收单完成签署',30004,30,'MANUAL',90,'集群已完成验收','COMPLETED',1,'admin',NOW());
INSERT INTO `lab_task` (`id`,`parent_id`,`goal_id`,`milestone_id`,`task_level`,`period`,`biz_line`,`task_type`,`title`,`owner_id`,`dept_id`,`plan_date`,`actual_finish_time`,`deliverable`,`perf_weight`,`goal_weight`,`workflow_status`,`result_status`,`execution_status`,`result_desc`,`fail_reason`,`next_action`,`asset_id`,`coordination_required`,`coordination_owner_id`,`coordination_dept_id`,`coordination_content`,`coordination_support`,`coordination_desc`,`current_block_flag`,`current_block_start`,`period_lock_flag`,`version`,`create_by`,`create_time`) VALUES (30001,0,30001,30002,'month','2026-08','algorithm','key','发布模型评测基准',30002,101,'2026-08-20',NULL,'评测看板',30,25,'PENDING_REVIEW','ONTIME',NULL,'评测报告已提交',NULL,'完成质量门禁验收',30003,'1',30003,101,'平台指标支持','提供可观测数据','需要平台提供指标','0',NULL,'0',1,'admin',NOW()),(30002,30001,30001,30002,'week','2026-W32','algorithm','daily','优化评测提示词',30006,101,'2026-08-09','2026-08-07 16:00:00','提示词套件',10,8,'CONFIRMED','EXCEEDED','SELF_DONE','覆盖率提升 15%',NULL,'分享评测结论',30003,'0',NULL,NULL,NULL,NULL,NULL,'0',NULL,'0',1,'admin',NOW()),(30003,0,30001,30003,'month','2026-08','platform','key','发布模型服务 2.1 版本',30003,101,'2026-08-15',NULL,'发布说明',35,30,'ACTIVE','DOING',NULL,'灰度部署进行中',NULL,'完成负载测试',30002,'1',30005,101,'可观测能力集成','开放指标访问','协调可观测能力','1','2026-08-06 09:00:00','0',1,'admin',NOW()),(30004,30003,30001,30003,'week','2026-W32','platform','daily','建设延迟监控看板',30005,101,'2026-08-08',NULL,'监控看板',12,8,'ACTIVE','DOING','ACTIVE','等待指标标签确认','指标结构尚未统一','确认指标结构',30002,'1',30003,101,'指标结构评审','形成结构决策','评审指标结构','1','2026-08-05 10:00:00','0',1,'admin',NOW()),(30005,0,30001,30004,'month','2026-08','hardware','key','完成集群验收',30004,101,'2026-08-05','2026-08-04 15:00:00','签署验收单',28,25,'CONFIRMED','ONTIME',NULL,'8 个节点全部通过验收',NULL,'归档验收证据',30001,'0',NULL,NULL,NULL,NULL,NULL,'0',NULL,'0',1,'admin',NOW());
INSERT INTO `lab_task` (`id`,`parent_id`,`goal_id`,`milestone_id`,`task_level`,`period`,`biz_line`,`task_type`,`title`,`owner_id`,`dept_id`,`plan_date`,`actual_finish_time`,`deliverable`,`perf_weight`,`goal_weight`,`workflow_status`,`result_status`,`execution_status`,`result_desc`,`fail_reason`,`next_action`,`asset_id`,`coordination_required`,`coordination_owner_id`,`coordination_dept_id`,`coordination_content`,`coordination_support`,`coordination_desc`,`current_block_flag`,`current_block_start`,`period_lock_flag`,`version`,`create_by`,`create_time`) VALUES (30006,0,30001,30002,'month','2026-08','algorithm','daily','完善模型评测自动化',30006,101,'2026-08-28',NULL,'自动化评测记录',0,0,'ACTIVE','DOING',NULL,'持续完善自动化覆盖',NULL,'拆解每周评测承诺',30003,'0',NULL,NULL,NULL,NULL,NULL,'0',NULL,'0',0,'admin',NOW());
INSERT INTO `lab_task_evidence` (`id`,`task_id`,`evidence_type`,`evidence_title`,`evidence_url`,`evidence_json`,`submitter_id`,`submit_time`,`audit_status`,`auditor_id`,`audit_time`,`audit_comment`,`create_by`,`create_time`) VALUES (30001,30001,'DOCUMENT','八月模型评测报告','https://example.invalid/evidence/benchmark',JSON_OBJECT('version','1.0'),30002,NOW(),'PENDING',NULL,NULL,NULL,'admin',NOW()),(30002,30005,'FILE','集群验收单','https://example.invalid/evidence/acceptance',JSON_OBJECT('nodes',8),30004,NOW(),'APPROVED',30001,NOW(),'验收通过','admin',NOW());
INSERT INTO `lab_task_quality_gate` (`id`,`task_id`,`gate_no`,`gate_name`,`gate_status`,`evidence_id`,`checker_id`,`check_time`,`check_result`,`create_by`,`create_time`) VALUES (30001,30001,'QG-01','评测结果可复现','PENDING',NULL,30001,NULL,NULL,'admin',NOW()),(30002,30005,'QG-01','硬件验收','PASSED',30002,30001,NOW(),'全部节点通过验收','admin',NOW());
INSERT INTO `lab_task_block_event` (`id`,`task_id`,`episode_no`,`block_type`,`block_reason`,`block_start_time`,`block_end_time`,`block_status`,`resolver_id`,`resolution`,`create_by`,`create_time`) VALUES (30001,30003,1,'DEPENDENCY','可观测指标尚未定稿','2026-08-06 09:00:00',NULL,'OPEN',NULL,NULL,'admin',NOW()),(30002,30004,1,'DEPENDENCY','指标标签仍待确认','2026-08-05 10:00:00',NULL,'OPEN',NULL,NULL,'admin',NOW());
INSERT INTO `lab_reminder` (`id`,`task_id`,`business_type`,`business_id`,`episode_no`,`recipient_id`,`reminder_type`,`reminder_level`,`reminder_date`,`title`,`reminder_content`,`read_flag`,`send_time`,`idempotency_key`,`version`,`create_by`,`create_time`) VALUES (30001,30003,'TASK',30003,1,30003,'BLOCK','WARNING','2026-08-13','任务阻塞提醒','模型服务发布被指标问题阻塞','0',NOW(),'BLOCK:30003:1:WARNING:30003:2026-08-13',0,'admin',NOW()),(30002,30001,'TASK',30001,NULL,30001,'REVIEW','INFO','2026-08-07','任务待验收','模型评测结果等待验收','0',NOW(),'REVIEW:30001:30001:2026-08-07',0,'admin',NOW());
INSERT INTO `lab_one2one` (`id`,`member_id`,`leader_id`,`meeting_date`,`topic`,`facts_evidence`,`difficulties`,`next_action`,`manager_comment`,`status`,`create_by`,`create_time`) VALUES (30001,30006,30002,'2026-08-07','评测能力成长','实验记录完整','跨团队数据集访问较慢','沉淀提示词方法文档','保持证据链并申请数据访问','OPEN','admin',NOW());
INSERT INTO `lab_ipr` (`id`,`ipr_no`,`ipr_name`,`ipr_type`,`ipr_stage`,`owner_id`,`planned_submit_date`,`actual_submit_date`,`acceptance_no`,`certificate_no`,`authorized_date`,`evidence_url`,`status`,`create_by`,`create_time`) VALUES (30001,'IPR-2026-01','模型评测流水线软件','SOFTWARE_COPYRIGHT','SUBMITTED',30002,'2026-07-31','2026-07-25',NULL,NULL,NULL,'https://example.invalid/ipr/1','ACTIVE','admin',NOW()),(30002,'IPR-2026-02','推理路由方法专利','PATENT','DRAFT',30003,'2026-10-31',NULL,NULL,NULL,NULL,NULL,'ACTIVE','admin',NOW());
...
INSERT INTO `lab_report_template` (`id`,`template_code`,`template_name`,`period_type`,`revision_no`,`latest_flag`,`default_flag`,`status`,`header_json`,`style_json`,`version`,`create_by`,`create_time`,`remark`) VALUES (30001,'standard_month','实验室标准月报','MONTH',1,'1',@ailab_seed_month_default_flag,'ENABLED',JSON_OBJECT('title','人工智能实验室月报','logo','ai-lab'),JSON_OBJECT('theme','blue','font','Microsoft YaHei'),1,'admin',NOW(),'默认月报模板');
INSERT INTO `lab_report_section` (`id`,`template_id`,`section_code`,`section_name`,`section_type`,`sort_no`,`data_source`,`query_config_json`,`render_config_json`,`style_config_json`,`manual_flag`,`visible_flag`,`sensitive_flag`,`sensitive_permission`,`version`,`create_by`,`create_time`) VALUES (30001,30001,'TASK_TABLE','任务交付','TABLE',10,'TASK_DETAIL',JSON_OBJECT('filters',JSON_ARRAY(JSON_OBJECT('field','period','operator','EQ','value','${period}'))),JSON_OBJECT('columns',JSON_ARRAY('owner','status','deliverable')),JSON_OBJECT('width','100%'),'0','1','0',NULL,1,'admin',NOW()),(30002,30001,'SCORE_STAT','绩效概览','STAT',20,'TASK_STAT',JSON_OBJECT('filters',JSON_ARRAY(JSON_OBJECT('field','period','operator','EQ','value','${period}'))),JSON_OBJECT('metrics',JSON_ARRAY('average','top')),JSON_OBJECT(),'0','1','1','lab:report:sensitive',1,'admin',NOW()),(30003,30001,'MANAGER_TEXT','目标进展','TEXT',30,'GOAL_PROGRESS',JSON_OBJECT('filters',JSON_ARRAY()),JSON_OBJECT('template','目标进展摘要'),JSON_OBJECT(),'0','1','0',NULL,1,'admin',NOW()),(30004,30001,'MANUAL_NOTE','管理小结','MANUAL',40,NULL,JSON_OBJECT('filters',JSON_ARRAY()),JSON_OBJECT('placeholder','填写本月管理小结','required',true),JSON_OBJECT(),'1','1','0',NULL,1,'admin',NOW()),(30005,30001,'LINE_GROUP','业务线进展','GROUP_TEXT',50,'TASK_COORD',JSON_OBJECT('filters',JSON_ARRAY()),JSON_OBJECT('groupBy','bizLine','template','业务线进展'),JSON_OBJECT(),'0','1','0',NULL,1,'admin',NOW()),(30006,30001,'PROGRESS_CHART','三季度目标进度','CHART',60,'GOAL_PROGRESS',JSON_OBJECT('filters',JSON_ARRAY()),JSON_OBJECT('chart','bar'),JSON_OBJECT(),'0','1','0',NULL,1,'admin',NOW());
INSERT INTO `lab_report_summary` (`id`,`period`,`biz_line`,`section_code`,`summary_json`,`summary_text`,`source_revision`,`create_by`,`create_time`) VALUES (30001,'2026-07','ALL','MANUAL_NOTE',JSON_OBJECT('bizLineSummary','七月绩效校准已完成','reasonAnalysis','交付证据和跨业务线评审已闭环','nextStep','推进三季度里程碑执行'),'七月管理小结',1,'admin',NOW()),(30002,'2026-08','platform','TASK_TABLE',JSON_OBJECT('active',2,'blocked',2),'平台发布需要协调指标口径',1,'admin',NOW());
INSERT INTO `lab_report_instance` (`id`,`report_no`,`template_id`,`template_code`,`template_revision`,`period`,`biz_line`,`revision_no`,`lifecycle_status`,`current_flag`,`final_flag`,`sensitive_flag`,`source_type`,`source_data_json`,`source_perf_revision`,`content_json`,`content_markdown`,`json_status`,`json_path`,`json_error`,`markdown_status`,`markdown_path`,`markdown_error`,`word_status`,`word_path`,`word_error`,`pdf_status`,`pdf_path`,`pdf_error`,`version`,`create_by`,`create_time`) VALUES (30001,'RPT-2026-07-ALL',30001,'standard_month',1,'2026-07','ALL',1,'FINALIZED','1','1','1','AUTO',JSON_OBJECT('performancePins',JSON_ARRAY(JSON_OBJECT('memberId',30002,'revisionNo',1),JSON_OBJECT('memberId',30003,'revisionNo',1),JSON_OBJECT('memberId',30005,'revisionNo',1))),1,JSON_OBJECT('sections',6),'# 人工智能实验室月报\n七月已关期报告','SUCCESS','archive/report-30001/report-30001.json',NULL,'SUCCESS','archive/report-30001/report-30001.md',NULL,'SUCCESS','archive/report-30001/report-30001.docx',NULL,'SUCCESS','archive/report-30001/report-30001.pdf',NULL,1,'admin',NOW());
...
(30001,'实验室阻塞扫描','DEFAULT','labScheduleTask.scanBlocks()','0 */15 * * * ?','3','1','0','admin',NOW(),'实验室管理'),(30002,'实验室待办扫描','DEFAULT','labScheduleTask.scanPendingTasks()','0 0 9 * * ?','3','1','0','admin',NOW(),'实验室管理'),(30003,'实验室周期关期','DEFAULT','labScheduleTask.closeDuePeriods()','0 0 2 1 * ?','3','1','0','admin',NOW(),'实验室管理'),(30004,'实验室报告临时文件清理','DEFAULT','labScheduleTask.cleanReportTempFiles()','0 0 2 * * ?','3','1','0','admin',NOW(),'实验室管理'),(30005,'实验室报告任务恢复','DEFAULT','labScheduleTask.recoverReportJobs()','0 */10 * * * ?','3','1','0','admin',NOW(),'实验室管理');
