...
public class LabPerformanceServiceImpl implements LabPerformanceService {
    ...
    public LabCollaborationRecord createCollaboration(LabCollaborationRecord record,Long actorUserId){
        ...
        validateScoreSign(record.getCategory(),record.getSignedScore()); record.setFromMemberId(actor.getMemberId()); record.setReviewerId(null); record.setReviewTime(null); record.setReviewComment(null);
    ...
    public void reviewCollaboration(Long id,CollaborationReviewCommand command,Long actorUserId){
        ...
        LabAccessContext actor=access.context(actorUserId); requireManager(actor); LabCollaborationRecord record=requireCollaboration(mapper.selectCollaborationForUpdate(id));
    ...
    public List<LabPerfScore> closePeriod(String period,String reason,Long actorUserId){
        ...
        LabPeriodClose close=mapper.selectPeriodForUpdate(period); if(close==null){mapper.ensureOpenPeriod(period,actor(actorUserId));close=mapper.selectPeriodForUpdate(period);}
        ...
        Date cutoff=Date.from(clock.instant()); List<LabTask> tasks=safe(mapper.selectPeriodTasksForUpdate(period)); List<LabMember> members=safe(mapper.selectActiveMembersForUpdate());
        assertStable(tasks,members); List<Long> taskIds=new ArrayList<Long>();for(LabTask task:tasks)taskIds.add(task.getId());
        List<LabTaskEvidence> evidence=safe(mapper.selectEvidenceForTaskIds(taskIds)); List<LabTaskQualityGate> gates=safe(mapper.selectQualityGatesForTaskIds(taskIds));
        List<LabCollaborationRecord> collaboration=new ArrayList<LabCollaborationRecord>(safe(mapper.selectCollaborationForPeriod(period)));
        String[] quarter=quarterRange(period); List<PerformanceAssetFact> assetFacts=safe(mapper.selectCriticalAssetFactsForUpdate(quarter[0],quarter[1]));
        ...
        for(LabMember member:members){
            PerformanceCalculationInput input=input(member.getId(),period,true,cutoff,tasks,evidenceMap,gateMap,collaboration,assetFacts); PerformanceCalculationResult result=calculator.calculate(input);
    ...
    public void confirmMonthlyScore(Long id,Integer version,Long actorUserId){
        ...
        if(!LabConstants.YES.equals(score.getCurrentFlag()))throw new ServiceException("Only current monthly revision may be confirmed");if(!isManager(actor)&&!same(actor.getMemberId(),score.getMemberId()))throw new ServiceException("Members may confirm only their own score");
    ...
    public LabPerfScore calibrateQuarter(String quarter,Long memberId,CalibrationCommand command,Long actorUserId){
        String[] range=quarterRangeFromQuarter(quarter);if(memberId==null||command==null||command.getScore()==null||command.getScore().compareTo(BigDecimal.ZERO)<0||command.getScore().compareTo(new BigDecimal("100"))>0||!hasText(command.getComment())||!validResultStatus(command.getResultStatus()))throw new ServiceException("Quarter calibration requires member, score 0-100, comment and valid result status");
        ...
        if(activeMonthlyRedLine&&LabConstants.PERF_RESULT_NORMAL.equals(command.getResultStatus()))throw new ServiceException("Active red line cannot be calibrated as normal");
        ...
        score.setDetailJson(JSON.toJSONString(detail));score.setCalculationVersion("AILAB_QUARTER_CALIBRATION_V1");score.setCutoffTime(Date.from(clock.instant()));score.setResultStatus(command.getResultStatus());score.setRedLineFlag(activeMonthlyRedLine?LabConstants.YES:LabConstants.NO);score.setRedLineReason(activeMonthlyRedLine?"ACTIVE_MONTHLY_RED_LINE":null);score.setRevokedFlag(LabConstants.NO);
    ...
    private PerformanceCalculationInput loadInput(Long memberId,String period,boolean close,Date cutoff){List<LabTask> tasks=safe(mapper.selectPeriodTasks(period));List<Long> ids=new ArrayList<Long>();for(LabTask task:tasks)ids.add(task.getId());String[] q=quarterRange(period);return input(memberId,period,close,cutoff,tasks,evidenceMap(safe(mapper.selectEvidenceForTaskIds(ids))),gateMap(safe(mapper.selectQualityGatesForTaskIds(ids))),safe(mapper.selectCollaborationForPeriod(period)),safe(mapper.selectCriticalAssetFacts(q[0],q[1])));}
    private PerformanceCalculationInput input(Long memberId,String period,boolean close,Date cutoff,List<LabTask> tasks,Map<Long,List<LabTaskEvidence>> evidence,Map<Long,List<LabTaskQualityGate>> gates,List<LabCollaborationRecord> collaboration,List<PerformanceAssetFact> assets){PerformanceCalculationInput in=new PerformanceCalculationInput();in.setMemberId(memberId);in.setPeriod(period);in.setCloseMode(close);in.setCutoffTime(cutoff);in.setTasks(tasks);in.setEvidenceByTask(evidence);in.setQualityGatesByTask(gates);in.setCollaborationRecords(collaboration);in.setAssetFacts(assets);return in;}
    ...
    private boolean validResultStatus(String status){return LabConstants.PERF_RESULT_NORMAL.equals(status)||LabConstants.PERF_RESULT_RED_LINE.equals(status);}
