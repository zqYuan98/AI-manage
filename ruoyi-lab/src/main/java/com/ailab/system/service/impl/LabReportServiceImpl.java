...
public class LabReportServiceImpl implements LabReportService {
    ...
    @Override public List<String> bizLines(Long actorUserId){LabAccessContext actor=access.context(actorUserId);java.util.SortedSet<String> active=new java.util.TreeSet<String>();for(String value:safe(mapper.selectActiveBizLines()))if(value!=null&&value.matches("[A-Za-z0-9_-]{1,32}"))active.add(value);if("lab_lead".equals(actor.getRoleKey()))return actor.getBizLine()!=null&&active.contains(actor.getBizLine())?Collections.singletonList(actor.getBizLine()):Collections.<String>emptyList();List<String> result=new java.util.ArrayList<String>();result.add("ALL");result.addAll(active);return result;}
    ...
    @Override public List<ReportSummarySectionView> summarySections(String period,String bizLine,Long actorUserId){requireSummaryScope(period,bizLine,actorUserId,false);ReportPeriod parsed;try{parsed=ReportPeriod.parse(period);}catch(IllegalArgumentException ex){throw new ServiceException("Invalid report summary period or business line");}List<ReportSummarySectionView> result=new java.util.ArrayList<ReportSummarySectionView>();for(com.ailab.system.domain.LabReportSection section:safe(mapper.selectDefaultManualSections(parsed.getKind().name()))){ReportSectionConfig config=new ReportSectionConfig(section);result.add(new ReportSummarySectionView(section.getSectionCode(),section.getSectionName(),Boolean.TRUE.equals(config.getRenderConfig().get("required"))));}return result;}
    @Override @Transactional public LabReportSummary saveSummary(LabReportSummary summary,Long actorUserId){requireSummaryIdentity(summary,actorUserId);if(summary.getSummaryJson()==null||summary.getSummaryJson().getBytes(StandardCharsets.UTF_8).length>65536)throw new ServiceException("Report summary JSON is missing or too large");validateSummaryJsonStream(summary.getSummaryJson());ReportManualSummaryContract.Canonical canonical;try{canonical=ReportManualSummaryContract.parse(summary.getSummaryJson());}catch(IllegalArgumentException ex){throw new ServiceException(ex.getMessage());}lockAndCheckSummaryRevision(summary);summary.setSummaryJson(canonical.getJson());summary.setSummaryText(canonical.getText());summary.setId(null);summary.setSourceRevision(1);summary.setDelFlag("0");summary.setCreateBy(actor(actorUserId));int rows=mapper.upsertSummary(summary);if(rows<1||rows>2)throw new ServiceException("Report summary was not saved");LabReportSummary stored=mapper.selectSummary(summary.getPeriod(),summary.getBizLine(),summary.getSectionCode());if(stored==null||stored.getId()==null||stored.getSourceRevision()==null)throw new ServiceException("Report summary identity could not be reloaded");return stored;}
    @Override @Transactional public List<LabReportSummary> saveSummaries(List<LabReportSummary> summaries,Long actorUserId){if(summaries==null||summaries.size()>100)throw new ServiceException("Report summary batch is invalid");List<LabReportSummary> result=new java.util.ArrayList<LabReportSummary>();Set<String> identities=new java.util.HashSet<String>();for(LabReportSummary value:summaries){String identity=value==null?"":String.valueOf(value.getPeriod())+'\u0000'+String.valueOf(value.getBizLine())+'\u0000'+String.valueOf(value.getSectionCode());if(!identities.add(identity))throw new ServiceException("Duplicate report summary in batch");if(value!=null&&(value.getSummaryJson()==null||value.getSummaryJson().trim().isEmpty())){requireSummaryIdentity(value,actorUserId);LabReportSummary current=lockAndCheckSummaryRevision(value);if(current!=null&&mapper.deleteSummary(value.getPeriod(),value.getBizLine(),value.getSectionCode(),actor(actorUserId))!=1)throw new ServiceException("Report summary was not cleared");}else result.add(saveSummary(value,actorUserId));}return result;}
    private void requireSummaryIdentity(LabReportSummary summary,Long actorUserId){if(summary==null)throw new ServiceException("Report summary is required");requireSummaryScope(summary.getPeriod(),summary.getBizLine(),actorUserId,true);if(summary.getSectionCode()==null||!summary.getSectionCode().matches("[A-Za-z0-9_-]{1,64}"))throw new ServiceException("Invalid report section code");}
    private LabReportSummary lockAndCheckSummaryRevision(LabReportSummary summary){LabReportSummary current=mapper.selectSummaryForUpdate(summary.getPeriod(),summary.getBizLine(),summary.getSectionCode());Integer expected=summary.getSourceRevision();if(current==null){if(expected!=null&&expected.intValue()>0)throw new ServiceException("Report summary was changed; reload before saving");return null;}if(expected==null||!expected.equals(current.getSourceRevision()))throw new ServiceException("Report summary was changed; reload before saving");return current;}
    private void requireSummaryScope(String period,String bizLine,Long actorUserId,boolean write){try{ReportPeriod.parse(period);}catch(IllegalArgumentException ex){throw new ServiceException("Invalid report summary period or business line");}if(bizLine==null||!bizLine.matches("[A-Za-z0-9_-]{1,32}"))throw new ServiceException("Invalid report summary period or business line");LabAccessContext actor=access.context(actorUserId);if("lab_manager".equals(actor.getRoleKey())&&("ALL".equals(bizLine)||safe(mapper.selectActiveBizLines()).contains(bizLine)))return;if("lab_lead".equals(actor.getRoleKey())&&bizLine.equals(actor.getBizLine()))return;throw new ServiceException(write?"Only managers and the matching line lead may edit summaries":"Report summaries are outside the actor scope");}
    ...
    private Page<?> detachPage(){Page<?> requested=PageHelper.getLocalPage();if(requested!=null)PageHelper.clearPage();return requested;}
    private void restorePage(Page<?> requested){if(requested==null)return;Page<?> restored=PageHelper.startPage(requested.getPageNum(),requested.getPageSize(),requested.getOrderBy());restored.setReasonable(requested.getReasonable());restored.setPageSizeZero(requested.getPageSizeZero());}
