...
public class LabReportServiceImpl implements LabReportService {
    ...
    @Override public List<ReportStatusView> history(String period,String bizLine,Long actorUserId){Page<?> requestedPage=detachPage();LabReportAccessScope scope=access.reportScope(actorUserId);restoreHistoryPage(requestedPage);List<LabReportInstance> mapped=safe(mapper.selectReportHistory(period,bizLine,scope.isManager(),scope.getBizLine(),scope.isShareAllFinalizedNonSensitive(),scope.isSensitiveGranted()));List<ReportStatusView> result;if(mapped instanceof Page){Page<?> source=(Page<?>)mapped;Page<ReportStatusView> page=new Page<ReportStatusView>(source.getPageNum(),source.getPageSize());page.setTotal(source.getTotal());result=page;}else result=new java.util.ArrayList<ReportStatusView>();for(LabReportInstance value:mapped)result.add(ReportStatusView.from(value));return result;}
    ...
    private void restoreHistoryPage(Page<?> requested){if(requested==null)return;int size=Math.max(1,Math.min(100,requested.getPageSize()));Page<?> restored=PageHelper.startPage(requested.getPageNum(),size);restored.setReasonable(requested.getReasonable());restored.setPageSizeZero(false);}
