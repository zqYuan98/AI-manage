...
public final class JsonReportExporter implements ReportExporter {
    ...
    private void number(StringBuilder out, Number value) { if ((value instanceof Double && !Double.isFinite(value.doubleValue())) || (value instanceof Float && !Float.isFinite(value.floatValue()))) throw new IllegalArgumentException("JSON numbers must be finite"); if (value instanceof BigDecimal) out.append(((BigDecimal) value).toPlainString()); else if (value instanceof BigInteger || value instanceof Byte || value instanceof Short || value instanceof Integer || value instanceof Long) out.append(value.toString()); else out.append(BigDecimal.valueOf(value.doubleValue()).stripTrailingZeros().toPlainString()); }
