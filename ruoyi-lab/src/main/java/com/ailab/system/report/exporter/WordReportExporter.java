...
public final class WordReportExporter implements ReportExporter {
    ...
    private void table(XWPFDocument document, ReportSectionData section, Limits limits) throws IOException {
        ...
        for (Map<String, Object> row : section.getRows()) {
            if (++limits.rows > MAX_ROWS) throw new ReportExportException("DOCX row limit exceeded", false); XWPFTableRow target = table.createRow();
    ...
    private void preflight(ReportData data) throws IOException { Limits limits = new Limits(); addText(limits, "人工智能实验室月报"); addText(limits, data.getContext().getPeriod()); addText(limits, data.getContext().getBizLine()); for (ReportSectionData section : data.getSections()) { addText(limits, section.getTitle()); grid(limits, section); for (Map<String,Object> row : section.getRows()) { if (++limits.rows > MAX_ROWS) throw new ReportExportException("DOCX row limit exceeded", false); for (Map.Entry<String,Object> entry : row.entrySet()) { addText(limits, entry.getKey()); addText(limits, text(entry.getValue())); } } scan(limits, section.getSummary()); } }
    /** Mirrors the only render paths that allocate a Word table. */
    private void grid(Limits limits, ReportSectionData section) throws IOException { String type = section.getSectionType(); if ("TABLE".equals(type) || ("STAT".equals(type) && section.getSummary().get("text") == null)) { tableGrid(limits, strings(section.getSummary().get("headers")).size(), section.getRows().size()); return; } if ("CHART".equals(type) && png(section.getSummary().get("pngBase64")) == null) { int categories = strings(section.getSummary().get("categories")).size(); if (categories > 0) cells(limits, 2, plusOne(categories)); } }
    private void tableGrid(Limits limits, int columns, int dataRows) throws IOException { if (columns > 0 && dataRows > 0) cells(limits, columns, plusOne(dataRows)); }
