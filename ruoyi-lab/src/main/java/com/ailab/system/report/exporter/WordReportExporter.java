...
public final class WordReportExporter implements ReportExporter {
    ...
    private void preflight(ReportData data) throws IOException { Limits limits = new Limits(); addText(limits, "人工智能实验室月报"); addText(limits, data.getContext().getPeriod()); addText(limits, data.getContext().getBizLine()); for (ReportSectionData section : data.getSections()) { addText(limits, section.getTitle()); if ("TABLE".equals(section.getSectionType())) cells(limits, strings(section.getSummary().get("headers")).size(), section.getRows().size() + 1); for (Map<String,Object> row : section.getRows()) { if (++limits.rows > MAX_ROWS) throw new ReportExportException("DOCX row limit exceeded", false); for (Map.Entry<String,Object> entry : row.entrySet()) { addText(limits, entry.getKey()); addText(limits, text(entry.getValue())); } } scan(limits, section.getSummary()); } }
