...
public final class ReportFieldSpec {
    public enum Type { STRING, NUMBER, BOOLEAN, DATE, DATETIME }
    ...
    private static final Set<String> DATE_FIELDS = set("planDate","plannedSubmitDate","actualSubmitDate");
    private static final Set<String> DATETIME_FIELDS = set("blockStartTime");

    private final String name; private final Type type; private final Set<String> operators;
    private ReportFieldSpec(String name, Type type) { this.name=name; this.type=type; this.operators=type==Type.NUMBER||type==Type.DATE||type==Type.DATETIME?ORDERED:EQUALITY; }
    ...
    private static Type typeOf(String name){if(NUMBER_FIELDS.contains(name))return Type.NUMBER;if(BOOLEAN_FIELDS.contains(name))return Type.BOOLEAN;if(DATETIME_FIELDS.contains(name))return Type.DATETIME;if(DATE_FIELDS.contains(name))return Type.DATE;return Type.STRING;}

    public void validate(String operator,Object expected){
    ...
    private void validateScalar(Object value){
        ...
        if((type==Type.DATE||type==Type.DATETIME)&&value instanceof String&&validDateForField((String)value))return;
    ...
    Object normalize(Object value){
        ...
        if((type==Type.DATE||type==Type.DATETIME)&&value instanceof String&&validDateForField((String)value))return value;
    ...
    int compare(Object left,Object right){
        ...
        if(type==Type.DATETIME)return Instant.parse((String)left).compareTo(Instant.parse((String)right));
    ...
    private boolean validDateForField(String value){try{if(type==Type.DATETIME)Instant.parse(value);else LocalDate.parse(value);return true;}catch(RuntimeException invalid){return false;}}
