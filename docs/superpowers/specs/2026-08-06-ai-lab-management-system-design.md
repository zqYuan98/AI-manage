...
| `lab_perf_score` | 月度评分与季度校准快照 | 成员、周期、修订号、当前标志、交付/质量/协同/总分、红线、计算明细 JSON、负责人评论、校准分、状态；成员+周期+修订号唯一 |
| `lab_period_close` | 月结审计与锁定 | 周期、关闭状态、关闭/重开人和时间、重开原因、版本号；周期唯一 |
...
结果状态为 `DOING`、`EXCEEDED`、`ONTIME`、`DELAYED`、`UNDONE`。激活时为 `DOING`；提交结果时，`UNDONE` 由负责人明确选择，其他完成结果中 `EXCEEDED` 需要审核人确认，`ONTIME/DELAYED` 由服务端比较持久化字段 `actual_finish_time` 与计划日期得出，客户端不能伪造。`EXCEEDED`、`ONTIME`、`DELAYED` 都属于完成类结果，适用相同的结果说明与证据校验。`提交计划` 是 `DRAFT → ACTIVE`，`提交结果` 是 `ACTIVE → PENDING_REVIEW`，审核通过是 `PENDING_REVIEW → CONFIRMED`，退回是 `PENDING_REVIEW → ACTIVE`。
...
日常实时计算只读取 `CONFIRMED` 结果；未到期的 `ACTIVE` 任务不按未完成计分。月结由 Quartz 在次月首日 02:00 执行，也允许部门负责人手动提前执行：事务内创建/更新 `lab_period_close` 为 `CLOSED`，锁定该周期月任务，并为每名成员生成不可变的 `lab_perf_score.detail_json` 修订快照。快照把截止时仍为 `DRAFT`、`ACTIVE` 或 `PENDING_REVIEW` 的重点任务按 `UNDONE` 计算并生成一次逾期协同扣分记录，但**不**篡改这些任务的工作流状态或伪造审核人。关闭后该周期任务只读，页面明确显示“月结时未确认”。

迟交或纠错需要部门负责人填写原因并重开周期；重开操作写审计日志并解除任务锁，原绩效快照保留为非当前修订。任务按正常状态机提交和审核后再次月结，生成递增修订号的新快照并设为当前。报告实例始终固化其生成时引用的绩效修订号，因此重开不会静默改变历史报告。
