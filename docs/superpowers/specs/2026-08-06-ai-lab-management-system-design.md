...
在 RuoYi-Vue 3.8.x 中新增 `ruoyi-lab` 业务模块，复用用户、部门、角色、字典、文件、Quartz、日志和数据权限能力。前端仍为 Vue 2 + Element UI。
...
| `lab_task` | 月任务、周任务 | `parent_id`、`goal_id`、`milestone_id`、`task_level`、`period`、`biz_line`、`task_type`、`owner_id`、`plan_date`、`deliverable`、`weight`、`status`、结果/原因/协调/阻塞字段、`asset_id`；`version` 用于乐观锁 |
...
目标进度默认按子项权重自动汇总；负责人可填写进展说明，但不能直接绕过子项修改自动进度。无子项时允许维护目标自身进度，添加子项后切换为自动汇总。
...
| 表 | 作用 | 关键字段/约束 |
|---|---|---|
| `lab_report_template` | 模板元数据 | 名称、唯一编码、报告类型、页眉 JSON、样式 JSON、默认标志、状态、版本 |
...
| `lab_report_instance` | 报告版本归档 | 模板、周期、版本号、状态、`report_data_json`、Markdown 正文、三种文件路径、来源（自动/导入 Markdown）、生成人和时间；模板+周期+版本唯一 |
| `lab_report_job` | 生成任务状态 | 实例、状态、进度、错误摘要、开始/结束时间；避免长时间 HTTP 阻塞 |

JSON 配置在导入和保存时按白名单 Schema 校验；模板文本只开放预定义变量和安全内建函数，不允许访问任意 Java 对象。已被报告实例引用的模板按版本复制，修改配置不追溯影响旧报告。
...
2. 表单按状态渐进显示：完成时展示结果与证据，未完成时展示原因与下步行动，需协调时展示协调字段。
3. 提交时后端统一返回字段级错误列表；前端定位并高亮，不只弹出笼统错误。
4. 月度重点任务提交前校验本人该周期有效任务权重合计等于 100。
...
Quartz 每日运行幂等扫描：阻塞超过 7 天生成负责人提醒，超过 14 天生成负责人看板红色预警；同一任务同一等级每天最多记录一次提醒。月末前 3 天生成本人待填清单，前 1 天将未填汇总给负责人。首期使用若依站内通知和看板待办，不接入外部消息平台。
...
失败时实例保留为 `FAILED`，记录可读错误摘要和服务端详细日志；重复生成创建新版本，不覆盖定稿；同模板、周期的进行中任务加互斥锁，防止重复点击。临时文件在成功归档或失败后由定时任务清理。
...
部门负责人拥有全部实验室数据；骨干拥有本业务线任务读写和本人绩效读取；普通成员只读写本人任务、查看本人绩效。所有列表与导出都在服务/Mapper 层应用数据范围，禁止仅靠前端隐藏。报告中的 `PERF_SUMMARY` 章节需要额外权限，即使模板误配置也不能越权取数。
